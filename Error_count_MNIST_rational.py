...
def Plot_K():
    ...
    for i in range(len(K)):
        print("K="+str(K[i]))
        success_rate[i]=Acc_Comparison(K[i],K[i]+S+2*e_max+1,S,num_of_iterations,actual_num_0f_errors)


    train_your_nicest_model(1)

    np.savetxt("Berrut_acc.txt",success_rate)
    np.savetxt("K.txt",K)
    plt.plot(K,success_rate,label="Berrut")
    plt.legend(["Berrut"])
    plt.xlabel("K")
    plt.ylabel("success_rate")
