...
def median_decoder(y,retuned_points_indices,actual_num_0f_errors):  # Returns the error locations
    alpha = parameters.alpha
    all_z = parameters.z_bar
    ...
    # print(yy)
    yy=-yy
    idx = np.argpartition(yy.transpose(), actual_num_0f_errors)
...
def decoder(Y,returned_points_indices,actual_num_0f_errors):
    ...
    #Y[erroneous_indices, :] = -Y[erroneous_indices, :]
    #Y[erroneous_indices, :] = -Y[erroneous_indices, :]
    ...
    for i in range(num_of_classes):
        ...
        # print(median_decoder(y,K,N,returned_points_indices).shape)
        # print(adversary_indices_matrix[:,i].shape)
        adversary_indices_matrix[:,i]=median_decoder(y,returned_points_indices,actual_num_0f_errors)
...
def Plot_K():
    ...
    for i in range(len(K)):
        print("K="+str(K[i]))
        success_rate[i]=Acc_Comparison(K[i],K[i]+S+2*e_max,S,num_of_iterations,actual_num_0f_errors)
