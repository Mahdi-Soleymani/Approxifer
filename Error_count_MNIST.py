...
def welch_decoder(y, K, N, retuned_points_indices):  # Returns the error locations
    ...
    A_psudo_inv = np.linalg.pinv(A,rcond=1e-15)
...
def decoder(Y,K,N,returned_points_indices):
    ...
    # print(np.sort(error_locations_predicted))
    # print(np.sort(erroneous_indices))
...
def Acc_Comparison(K, N, S, iterations):
    ...
    for i in range(iterations):
        ...
        success_rate = success_rate+decoder(model_outputs, K, N, returned_points_indices)
...
def Plot_N():
    ...
    for i in range(len(N)):
        print("N="+str(N[i]))
        success_rate[i]=Acc_Comparison(K,N[i],S,num_of_iterations)


    np.savetxt("success_rate.txt", success_rate)
    np.savetxt("N.txt", N)
    plt.plot(N, success_rate, label="success_rate")
...
def Plot_K():
    ...
    for i in range(len(K)):
        print("K="+str(K[i]))
        success_rate[i]=Acc_Comparison(K[i],2*K[i]+S+2*e_max-1,S,num_of_iterations)
    ...
    plt.xlabel("K")
    plt.ylabel("success_rate")
    plt.title('N=2K+S+2E-1, S=' +str(S)+',E=' +str(e_max)+', Num_of_Iterations='+ str(num_of_iterations))
    plt.show()
