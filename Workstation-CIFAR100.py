...
#model = torch.hub.load("chenyaofo/pytorch-cifar-models", "cifar100_resnet151", pretrained=True)
...
def Determine_accuracy(input_batch_ids):####only used for centralized
...
def Plot_K():
    ...
    model_name=model.__class__.__name__+'152'
    ...
    np.savetxt('N='+str(coeff)+str(K)+'K='+str(K)+'S='+str(S)+model_name+'iterations'+str(num_of_iterations)+"top"+str(top_what_accuracy)+"Berrut_acc.txt",Berrut_acc)
    np.savetxt('N='+str(coeff)+str(K)+'K='+str(K)+'S='+str(S)+model_name+'iterations'+str(num_of_iterations)+"top"+str(top_what_accuracy)+"Center_acc.txt",Center_acc)
    np.savetxt('N='+str(coeff)+str(K)+'K='+str(K)+'S='+str(S)+model_name+'iterations'+str(num_of_iterations)+"top"+str(top_what_accuracy)+"K.txt",K)
...
# pred=torch.tensor([1,2,3,0,4,5,6,7,8])
#
# print(pred)
# _, predicts = torch.topk(pred,2)
# print(predicts)
