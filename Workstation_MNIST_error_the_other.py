...
def gt(x):
    out=0
    for i in range(parameters.emax):
...
def welch_decoder(y,retuned_points_indices,actual_num_0f_errors):  # Returns the error locations
    ...
    for i in range(len(returned_z)):
        for j in range(parameters.K):
            A1[i,j]=1/((returned_z[i]-alpha[j])*G(returned_z[i]))



    A2=np.zeros([len(returned_z),parameters.emax])
    for i in range(len(returned_z)):
        for j in range(parameters.emax):
            A2[i,j]=1/((returned_z[i]-parameters.beta[j])*G(returned_z[i]))


    A3=np.zeros([len(returned_z),parameters.emax])
    for i in range(len(returned_z)):
        for j in range(parameters.emax):
    ...
    #print(b)
    lambda_evals = np.ones([parameters.N, 1]) ### 1 is because locator function is 1+ ...


    for i in range(parameters.N):
        for j in range(parameters.K):

            lambda_evals[i] = lambda_evals[i] + ((a[j]) / ((all_z[i] - parameters.alpha[j])))

        for j in range(parameters.emax):

            lambda_evals[i] = lambda_evals[i] + ((a[j+parameters.K]) / ((all_z[i] - parameters.beta[j])))

        lambda_evals[i] = lambda_evals[i]/G(all_z[i] )
...
class parameter_container:
    ...
    def update_parameters(self, K, N, S):
        ...
        for i in range(self.emax):
            self.beta[i] = np.cos((3*i * np.pi+1) / (3*self.emax))  #####

        # self.beta= np.zeros(self.emax)
        # for i in range(self.emax):
        #     self.beta[i] = np.cos(((2 * i + 1) * np.pi) / (2 * self.emax)) #####


def plot_parameters():
    plt.plot(parameters.alpha,np.zeros((parameters.K)),'x')
    plt.plot(parameters.z_bar, np.zeros((parameters.N)), 'x')
    plt.plot(parameters.beta, np.zeros((parameters.emax)), 'x')
    plt.show()
...
def Plot_K():
    ...
    for i in range(len(K)):
        print("K="+str(K[i]))
        a,b=Acc_Comparison(K[i],K[i]+S+2*e_max,S,num_of_iterations,actual_num_0f_errors)
        Berrut_acc[i]=a
        Center_acc[i]=b
