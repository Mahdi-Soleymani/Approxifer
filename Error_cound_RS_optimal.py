...
def welch_decoder(y,retuned_points_indices,actual_num_0f_errors):  # Returns the error locations
    ...
    yprime=transformy(y,alpha,returned_z,parameters.K)
#######################################################
    #### use yprime instead of y



    A1 = np.vander(returned_z, N=parameters.K + parameters.emax)
    A2 = np.vander(returned_z, N= parameters.emax+1)
    A2 = -A2[:, 0:parameters.emax]
    A2 = A2 * yprime
    ...
    #print(coeffs)
    a = coeffs[0:parameters.K + parameters.emax, 0]
    b = coeffs[parameters.K + parameters.emax:parameters.K+2*parameters.emax, 0]
    b = np.reshape(b, [len(b), 1])
    bb = np.concatenate([b, np.ones([1, 1])])
    #plambda_evals = np.zeros([N, 1])
    qlambda_evals = np.zeros([parameters.N, 1])
    ...
    output = np.zeros([parameters.N, 1])
    ...
    idx = np.argpartition(np.abs(qlambda_evals.transpose()), parameters.emax)
    # print((idx[:, :e]))
    ...
    e=parameters.emax #see what happens in experiments
...
def decoder(Y,returned_points_indices,actual_num_0f_errors):
    ...
    counter=0
    for i in range(len(error_locations_predicted)):
        if error_locations_predicted[i] in erroneous_indices:
            counter=counter+1

    success_rate=counter/e_max
...
def Acc_Comparison(K, N, S, iterations,actual_num_0f_errors):
    ...
    for i in range(iterations):
        ...
        success_rate = success_rate+decoder(model_outputs, returned_points_indices,actual_num_0f_errors)
    ...
    return success_rate / iterations
...
def Plot_K():
    ...
    for i in range(len(K)):
        print("K="+str(K[i]))
        success_rate[i]=Acc_Comparison(K[i],2*K[i]+S+2*e_max,S,num_of_iterations,actual_num_0f_errors)


    #train_your_nicest_model(1)

    np.savetxt("success_rate.txt",success_rate)
    np.savetxt("K.txt",K)
    plt.plot(K,success_rate,label="success_rate")
    plt.legend(["Berrut","Centralized"])
    plt.xlabel("K")
    plt.ylabel("Accuracy")
